...
static NSString *ZRKeychainErrorDomain = @"ZRKeychainErrorDomain";


#if USE_MAC_KEYCHAIN_API

@interface ZRKeychain ()
...
{
    NSData *passwordData = [ZRKeychain getPasswordDataForUsername:username andServiceName:serviceName error:error];
...
+ (NSData *)getPasswordDataForUsername:(NSString *)username andServiceName:(NSString *)serviceName error:(NSError **)error
{
	if (!username || !serviceName) {
        if (error) {
            *error = [NSError errorWithDomain:ZRKeychainErrorDomain code:-2000 userInfo:nil];
        }
		
        return nil;
	}
	
	SecKeychainItemRef item = [ZRKeychain getKeychainItemReferenceForUsername:username andServiceName:serviceName error:error];
	if ((error && *error) || !item) {
		return nil;
	}
	...
	if (status != noErr) {
		if (error) {
            *error = [NSError errorWithDomain:ZRKeychainErrorDomain code:status userInfo:nil];
        }
...
{
    return [ZRKeychain storeUsername:username andPasswordData:[passwordString dataUsingEncoding:NSUTF8StringEncoding] forServiceName:serviceName updateExisting:updateExisting error:error];
}
...
+ (NSData *)getPasswordDataForUsername:(NSString *)username andServiceName:(NSString *)serviceName error:(NSError **)error
{
	...
	if (status != noErr) {
		if (status == errSecItemNotFound) {
			...
			if (error) {
				*error = [NSError errorWithDomain:ZRKeychainErrorDomain code:-1999 userInfo:nil];
			}
		} else if (error) {
...
{		
	...
	NSData *existingPassword = [ZRKeychain getPasswordDataForUsername:username andServiceName:serviceName error:&getError];
